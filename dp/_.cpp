...
int dp[2001][2001];
int n, m, k;
int rec(int level, int diffCount)
{
    if (level == n)
    {
        return diffCount == k;
    ...
    {
        return dp[level][diffCount];
    }
    int ans = 0;
    if (level == 0)
    {
        // there are m ways in which the first element can be placed
        ans += m * (rec(level + 1, diffCount) % mod);
        ans %= mod;
    }
    else
    {
        // right one placed the same as left one
        ans += rec(level + 1, diffCount) % mod;
        ans %= mod;

        // right one a different one than the left one
        ans += ((m - 1) * rec(level + 1, diffCount + 1)) % mod;
        ans %= mod;
    }

    return dp[level][diffCount] = ans;
    // dp(level, diffCount)-> m*dp(level+1, diffCount) ( since if level'th
    // color is same as level+1th career then any )
}
void solve()
{
    // Your code here
