...
void solve()
{
    ...
    vector<vector<int>> dist(n + 1, vector<int>(404, 1e18)), vis(n + 1, vector<int>(404, 0));
    ...
    while (!q.empty())
    {
        ...
        for (auto v : g[node])
        {
            ...
            if (tank >= ptl)
            {
                if (dist[ngh][tank - ptl] > curdist)
                {
                    dist[ngh][tank - ptl] = curdist;
        ...
        if (tank < k)
        {
            if (dist[node][tank + 1] > curdist + c[node])
            {
                dist[node][tank + 1] = curdist + c[node];
